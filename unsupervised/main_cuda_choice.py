...
parser.add_argument('--no-cuda', action='store_true',
                    help='do not run model with GPU support')
...
def main():
    ...
    if 'KITTI' in args.dataset:
        co_transform=flow_transforms.Compose([
            ...
            #random flpis are not supported yet for tensor conversion, but will be
    ...
    #need to uncomment this for quick test
    #train_set = torch.utils.data.TensorDataset(torch.rand(16, 6, 64, 64), torch.rand(16, 2, 64, 64))
    #test_set = torch.utils.data.TensorDataset(torch.rand(16, 6, 64, 64), torch.rand(16, 2, 64, 64))

    # need to comment this for quick test
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers=args.workers, pin_memory=not args.no_cuda)
    ...
    criterion = multiscaleloss(sparse = 'KITTI' in args.dataset, loss=args.loss)
    high_res_EPE = multiscaleloss(scales=1, downscale=4, weights=(1), loss='L1', sparse = 'KITTI' in args.dataset)
    
    if not args.no_cuda:
        model = model.cuda()
        model = torch.nn.DataParallel(model)
        criterion = criterion.cuda()
        high_res_EPE = high_res_EPE.cuda()
...
def train(train_loader, model, criterion, EPE, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if not args.no_cuda:
            ...
            input = torch.cat([j.cuda(0) for j in input])
