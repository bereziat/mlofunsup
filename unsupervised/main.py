...
def train(train_loader, model, criterion, EPE, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        target_var = stn_transform(output[0],input[0])

        loss = criterion(input[0], target_var)
...
def validate(val_loader, model, criterion, EPE):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        output = model(input_var)
        target_var = stn_transform(output[0],input[0])
        flow2_EPE = args.div_flow*EPE(input, target_var)
        # record EPE
        flow2_EPEs.update(flow2_EPE.data[0], target.size(0))
    ...
    for i in range(m):
        for j in range(n):
            if((abs(output[i][j][0])+abs(output[i][j][1])!=0)):
                imrec[i+math.floor(output[i][j][0])][j+math.floor(output[i][j][1])]=input[i][j]
    return imrec
